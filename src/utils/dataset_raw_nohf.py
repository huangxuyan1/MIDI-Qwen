...
from miditok.constants import CHR_ID_START, UNIGRAM_SPECIAL_TOKEN_SUFFIX


def init_hf_model_for_raw(tokenizer) -> None:
    """
    Initialize a HuggingFace tokenizer on top of a raw MidiTok tokenizer
    WITHOUT any BPE / Unigram training. This only exists so that code relying on
    `tokenizer._model.id_to_token` will work in the "raw" setting.
    """

    # 1. Safety: multi-vocab tokenizers can't be represented as a single HF model
    if tokenizer.is_multi_voc:
        warnings.warn(
            "This tokenizer is based on multiple vocabularies/embedding pooling. "
            "It cannot be converted to a single HF model.",
            stacklevel=2,
        )
        return

    # 2. Build the same initial alphabet as in MusicTokenizer.train(...)
    initial_alphabet = {
        chr(i + CHR_ID_START): i
        for tok, i in tokenizer._vocab_base.items()
        if not tok.endswith(UNIGRAM_SPECIAL_TOKEN_SUFFIX)
    }

    # 3. BPE model with NO merges -> identity over the initial alphabet
    model = _tok_models.BPE(
        vocab=initial_alphabet,
        merges=[],
    )
    hf_tokenizer = _HFTokenizer(model)

    # 4. Keep bar/beat splitting behavior consistent with MidiTok
    if tokenizer.config.encode_ids_split != "no":
        hf_tokenizer.pre_tokenizer = _pre_tokenizers.Metaspace()
        hf_tokenizer.decoder = _decoders.Metaspace()

    # 5. Attach HF tokenizer to the existing MidiTok tokenizer
    tokenizer._model = hf_tokenizer

    # 6. For your raw ablation, you're only using _model.id_to_token in the dataset.
    # You do NOT need learned-bytes mapping, so leaving this as {} is fine.
    tokenizer._vocab_learned_bytes_to_tokens = {}
    # If you ever want full decode/encode compatibility, you'd call:
    # tokenizer._MusicTokenizer__create_vocab_learned_bytes_to_tokens()
    # but only if you also set up _vocab_base_id_to_byte consistently.
...
class MIDIDataset(DatasetMIDI):
    ...
    **Handling of corrupted files:**
    ...
    def __init__(
        ...
        # After constructing your MidiTok tokenizer, before creating the dataset:
        if tokenizer._model is None:
            init_hf_model_for_raw(tokenizer)
